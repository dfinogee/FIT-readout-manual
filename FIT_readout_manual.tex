...
Регистр 'Max RDH payload' задает максимальное количество слов в RDH пакете. Максимальный объем RDH пакета составляет 8 Кб, поскольку каждое переданное GBT слово дополняется в CRU до 128 бит, максимум в RDH пакете может содержаться 8192*8/128=512 слов GBT. Значение регистра соответствует значению счетчика GBT слов при котором закрывается RDH и должно быть равно 0х1F4 (500). 500+7(максимальный размер события)+5(RDH заголовок, трейлер)=512. Для отладки максимальный размер пакета может быть уменьшен.
...
\subsection{Инициализация регистров при запуске электроники}

При запуске электроники или после сигнала reset необходимо установить значения по умолчанию в регистры управления что соответствует штатной работе пакета.
...
\begin{tabular}{| l | l | l |}
\hline
Название регисра & значение & комментарий \\ \hline
Readout mode & 0x5 & ответ HB включен; редаут выключен \\ \hline
Readout command & 0x0 &  \\ \hline
Reset control & 0х0 &  \\ \hline
Trigger generator & 0х0 & \\ \hline
Data generator & 0х0 &  \\ \hline
Trigger response mask & 0х0 &  \\ \hline
Data bunch pattern & 0х0 &  \\ \hline
Trigger single value & 0х0 &  \\ \hline
Trigger continuous pattern & 0х0 &  \\ \hline
Trigger continuous value & 0х0 &  \\ \hline
Trigger bunch frequency & 0х0 &  \\ \hline
Data bunch frequency & 0х0 &  \\ \hline
Trigger start offset & 0х0 &  \\ \hline
Data start offset & 0х0 & \\ \hline
FEE ID &  & согласно карте \\ \hline
PAR & 0х0 &  \\ \hline
Max RDH payload & 0x1F4  & см. модуль Event Selector; может быть увеличен на 1 после отключения RDH трейлера\\ \hline
Detector field & 0x0 & нет \\ \hline
Trigger compare delay &  0xf & будет уточнено после тестов на P2 \\ \hline
BCID delay & 0x20 & будет уточнено после тестов на P2, индивидуально для каждой платы \\ \hline
Data select trigger mask &  0х10 & отбор по физическому триггеру \\ \hline
%reg & module & да \\ \hline
\end{tabular}
\caption{Значения по умолчанию регистров управления для штатной работы\label{tab1}}
\end{table}



\section{Описание пакета симуляции}

Пакет симуляции выполнен на языке pyton и расположен в папке alice-fit-fpga/software/readout-sim. Симуляция позволяет выполнить проверку работы модуля FIT-readout-unit при симуляции vhdl кода пакетом Vivado (в настоящее время выполнено только логическая симуляция). Последовательность проверки следующая:

\begin{itemize}
\item Запись файла readout-sim/simulation\_inputs/simple\_sig\_inputs.txt который содержит конфигурацию всех регистров для каждого цикла @40МГц. Файл записывается в первой части исполняймого файла testbench.py и содержит команды firmware-генераторов для запуска и остановки записи данных в режимах которые необходимо проверить. На данный момент это triggered, continious, continious-load (несколько пакетов на орбит) раны.
\item Запуск симуляции в пакете Vivado. Симуляция использует файл simple\_sig\_inputs.txt для конфигурации пакета редаута. Как результат симуляции записывается файлы readout-sim/simulation\_outputs/readout\_gbt\_info\_output.txt, readout\_gbt\_output.txt, readout\_status\_reg\_output.txt. В файл readout\_status\_reg\_output.txt записывается расширенный регистр статуса (добавлены триггер, скорректированый ORBC и размер пакета в DataGenerator) для каждого цикла. В файл readout\_gbt\_output.txt записываются все ненулевые слова с GBT, а в readout\_gbt\_info\_output.txt номера циклов в которых записаны слова GBT.
\item Запуск проверки выходных файлов с выводом ошибок найденных в пакетах данных.
\end{itemize}

Запуск проверки данных запускается в исполняемом файле testbehch.py. В начале создается элемент класса simulation\_data\_class. В данном классе указываются пути для всех выходных файлов и файлы связываются с переменными типа list для чтения. При создании класса simulation\_data
\_class читается входной файл конфигурации simple\_sig\_inputs.txt и производится поиск команд на запуск и остановки ранов. Для каждого рана создается элемент класса run\_sim\_data\_class где читается из файла конфигурации вся необходимая информация о ране (начало и конец рана в файлах, тип рана и его конфигурация) и проверяется корректность регистров конфигурации во время рана (постоянность конфигурации, отсутствие ранов некоторое время до старта и после остановки). Так же при создании класса создается список с данными и триггерами генерированные в тестовых генераторах firmware.

После создания класса simulation\_data\_class создается элемент класса run\_testbench\_class для каждого рана, классу передается класс simulation\_data\_class и номер рана который необходимо проверить. При создании класса run\_testbench\_class производится чтение пакетов RDH и пакетов детектора из файла GBT  с проверкой ошибок в формате данных. После считывания, пакеты проверяются к корректность (наличие триггеров старта и остановки рана в данных, счетчик пакетов RDH, stop bit, orbit в заголовке RDH и событиях, наличие всех сгенерированных данных с учетом режима вычитываня continious/trigger). Статус выполнения проверки выводится логом в терминал и файл.
